  ...
  return habitaciones.value.filter((habitacion) => {
    const matchesSearch = searchTerm.value
      ? habitacion.numero.toString().includes(searchTerm.value) ||
        habitacion.vista?.toLowerCase().includes(searchTerm.value.toLowerCase()) ||
        habitacion.tipo.toLowerCase().includes(searchTerm.value.toLowerCase())
      : true;

    const matchesEstado =
      filterEstado.value === "todos" || habitacion.estado === filterEstado.value;
...
const loadHabitaciones = async () => {
  try {
    ...
    useToast().showMessage("error", "Error al cargar las habitaciones");
  } finally {
    loading.value = false;
  }
};
...
const saveHabitacion = async () => {
  try {
    if (!habitacionForm.value.numero || !habitacionForm.value.capacidad) {
      useToast().showMessage("error", "Número, tipo y capacidad son requeridos");
      return;
    }
    ...
    if (editingHabitacion.value) {
      ...
      if (error) throw error;
      useToast().showMessage("success", "Habitación actualizada exitosamente");
    } else {
      ...
      if (error) throw error;
      useToast().showMessage("success", "Habitación creada exitosamente");
    }
...
<template>
  <div>
    ...
    <table v-else class="admin-table">
      ...
      <tbody>
        <tr v-for="habitacion in filteredHabitaciones" :key="habitacion.id_habitacion">
          ...
          <td>
            <span :class="['estado-badge', habitacion.estado.toLowerCase()]">{{ habitacion.estado }}</span>
          </td>
    ...
    <div v-if="showModal" class="modal-backdrop" @click.self="closeModal">
      <div class="modal-card">
        <h2>{{ editingHabitacion ? 'Editar Habitación' : 'Crear Habitación' }}</h2>

        <form @submit.prevent="saveHabitacion">
          <div class="field-row">
            <label>Número *</label>
            <input type="number" v-model.number="habitacionForm.numero" min="1" required />
          </div>
          ...
          <div class="field-row">
            ...
            <input type="number" v-model.number="habitacionForm.capacidad" min="1" required />
          </div>

          <div class="field-row">
            <label>Piso *</label>
            <input type="number" v-model.number="habitacionForm.piso" min="1" required />
          </div>
          ...
          <div class="field-row">
            ...
            <input type="number" v-model.number="habitacionForm.precio_noche" min="0" step="0.01" required />
          </div>
